...
clinical_cleaned = args[2]
if (file.exists(clinical_cleaned)) {
...
clinicals <- read.table(clinical_cleaned,
                        sep=",", header=TRUE, stringsAsFactors = FALSE)

clinicals[,c("Start.Date", "Completion.Date","Last.Updated", "Primary.Completion.Date")] <- lapply(clinicals[,c("Start.Date", "Completion.Date","Last.Updated", "Primary.Completion.Date")], function(x) as.Date(x, "%Y-%m-%d"))
clinicals$Phases <- ifelse(clinicals$Phases == "", "preclinical", clinicals$Phases)
#clinicals[, c("Phases")] <- clinicals[,c("Phases")]
clinicals$Title <- str_wrap(clinicals$Title, width = 55)
clinicals$Progress <- ""
